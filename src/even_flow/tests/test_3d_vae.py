...
Separable convolutions should implement a 2D conv followed by a 1D conv. Make sure these are tested in each model.
"""
...
CONFIGS = [
    ...
    ConvolutionalVariationalAutoencoderConfig(
        ...
        encoder_layers=[
            DownsampleConvLayerConfig(dim=3, in_channels=3, out_channels=16, kernel_size=3, downsample_method="max"),
            ActivationLayerConfig(activation="GELU"),
            DownsampleConvLayerConfig(dim=3, in_channels=16, out_channels=32, kernel_size=3, downsample_method="avg", separable=True),
            ActivationLayerConfig(activation="ReLU"),
            DownsampleConvLayerConfig(dim=3, in_channels=32, out_channels=64, kernel_size=3, downsample_method="max"),
            ActivationLayerConfig(activation="SiLU"),
        ],
        probabilistic_layer=ProbabilisticLayerConfig(dim=3, latent_dim=64, channels=64),
        decoder_layers=[
            UpsampleConvLayerConfig(dim=3, in_channels=64, out_channels=32, kernel_size=3, upsample_method="nearest"),
            ActivationLayerConfig(activation="GELU"),
            UpsampleConvLayerConfig(dim=3, in_channels=32, out_channels=16, kernel_size=3, upsample_method="trilinear", separable=True),
            ActivationLayerConfig(activation="GELU"),
            UpsampleConvLayerConfig(dim=3, in_channels=16, out_channels=3, kernel_size=3, upsample_method="nearest"),
